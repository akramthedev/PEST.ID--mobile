...
const ModifierSerre = () => {
  ...
  useEffect(()=>{
    const x = async ()=>{
      if(name && farmId && serreId){
        const rolex = JSON.parse(await AsyncStorage.getItem('type'));
        setRole(rolex);
        setIdFarm(farmId);setIdSerre(serreId);setTypeS(type === null || type === "" ? null : type);setSizeS(parseFloat(size));setNameS(name)
        setLoading(false);
      }
    }
    x();
  ...
  const handleUpdate = async ()=>{
    if(IdSerre !== null && IdSerre !== undefined){
      try{
        ...
        let datak = {
          name : nameS, 
          size : sizeS === null || sizeS === undefined || sizeS === 0 ? 0 : sizeS,  
          type : typeS === "Veuillez saisir la valeur..." || typeS === null || typeS === "" ? null : typeS
        }
