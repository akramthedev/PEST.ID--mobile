...
const CreateStaff = ({route}) => {
  ...
  const validateFields = () => {
    ...
    if (!fullname) missingFields.push("nom");

    if (missingFields.length > 0) {
      setmessageError(`Le${missingFields.length >1 ? "s" : "" } champ${missingFields.length >1 ? "s" : "" } ${missingFields.join(", ")} ne ${missingFields.length >1 ? "peuvent" : "peut" } pas être vide${missingFields.length >1 ? "s" : "" }.`);
      setShowError(true);
      return false;
    }
    return true;
